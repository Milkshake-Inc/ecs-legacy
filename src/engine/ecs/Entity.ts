...
export class Entity {
	...
	public addComponent<T extends K, K extends any>(component: T, resolveClass?: Class<K>): Entity {
		...
		if (resolveClass) {
			if (!((component as any) instanceof resolveClass && componentClass != resolveClass)) {
...
export class EntitySnapshot {
	...
	public has<T>(componentClass: Class<T>): boolean {
		...
		return componentId !== undefined && this._components !== undefined && this._components.has(componentId);
	}
}
