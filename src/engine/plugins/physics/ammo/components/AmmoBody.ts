...
export default class AmmoBody {
	...
	public setPosition(vector: Vector) {
		...
		this.body.getWorldTransform().setOrigin(new AmmoInstance.btVector3(vector.x, vector.y, vector.z));
	}

	public applyCentralImpulse(vector: Vector) {
		this.body.activate(true);

		this.body.applyCentralImpulse(new AmmoInstance.btVector3(vector.x, vector.y, vector.z));
	}
}
